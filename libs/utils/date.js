...
fecha.format = function (dateObj, mask, i18nSettings) {
  ...
  let formatMask = fecha.masks[mask] || mask || fecha.masks['default'];

  return formatMask.replace(token, function ($0) {
    return $0 in formatFlags ? formatFlags[$0](newDate, i18n) : $0.slice(1, $0.length - 1);
  });
};
...
fecha.parse = function (dateStr, format, i18nSettings) {
  ...
  let _format = fecha.masks[format] || format;

  // Avoid regular expression denial of service, fail early for really long strings
  // https://www.owasp.org/index.php/Regular_expression_Denial_of_Service_-_ReDoS
  if (tempDate.length > 1000) {
    return false;
  }
  ...
  _format.replace(token, function ($0) {
    if (parseFlags[$0]) {
      ...
      var index = tempDate.search(info[0]);
      if (!~index) {
        isValid = false;
      } else {
        tempDate.replace(info[0], function (result) {
          info[1](dateInfo, result, i18n);
          tempDate = tempDate.substr(index + result.length);
          return result;
        });
      }
    }
