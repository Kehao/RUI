...
const Print = options => {
  ...
  const markLoaded = (linkNode, loaded) => {
    if (loaded) {
      ...
      // eslint-disable-next-line no-console
      console.error(
        'unable to load a link. It may be invalid.We will continue attempting to print the page. The link the errored was:',
        linkNode
      ...
      [...headEls].forEach((node, index) => {
        if (node.tagName === 'STYLE') {
          ...
          if (hasHref) {
            ...
            // eslint-disable-next-line no-console
            console.warn(
              "We encountered a <link> tag with an empty 'href' attribute. In addition to being invalid HTML, this can cause problems in many browsers, and so the <link> was not loaded. The <link> is:",
              node
            );
